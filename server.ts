...
async function startServer() {
  ...
  wss.on('connection', (ws, req) => {
    ...
    // Initialize doc content from FS if available
      ...
      if (roomName && filePath) {
          ...
          if (!fullPath.startsWith(safeRoot + path.sep)) {
              console.error(`[Yjs] Invalid path detected: ${fullPath}`);
              ws.close();
              return;
          }
      ...
      doc.on('update', (update: Uint8Array, origin: any) => {
        ...
        if (roomName && filePath) {
            ...
            if (!fullPath.startsWith(safeRoot + path.sep)) {
                console.error(`[Yjs] Invalid path detected during save: ${fullPath}`);
                return;
            }
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('editor:create', ({ roomName, node }: { roomName: string; node: EditorNode }) => {
        ...
        if (!fullPath.startsWith(roomDir)) {
    ...
    socket.on('editor:rename', ({ roomName, nodeId, newName }: { roomName: string; nodeId: string; newName: string }) => {
        ...
        if (!oldPath.startsWith(roomDir) || !newPath.startsWith(roomDir)) {
            console.error("Invalid path detected in editor:rename");
            return;
        }
